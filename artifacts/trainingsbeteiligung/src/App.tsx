...
import {
  ...
  Download, Menu, Plus, Settings, Shield, Trash2, Users, Upload, X,
} from 'lucide-react';

type Status = 'present' | 'excused' | 'unexcused' | 'injured';
type Player = { id: string; name: string; activeFrom: string; activeTo: string };
type Training = { id: string; date: string; note: string; attendance: Record<string, Status>; teamGroups?: string[][] };
...
function TrainingModal({team,date,training,close,save,remove}:{team:Team;date:string;training:Training|null;close:()=>void;save:(t:Training)=>void;remove?:()=>void}) {
  ...
  const [teamCount,setTeamCount]=useState<2|3>(()=>training?.teamGroups?.length===3?3:2);
  const [groups,setGroups]=useState<string[][]>(()=>training?.teamGroups??[[],[]]);
  const presentPlayers=active.filter(p=>attendance[p.id]==='present');
  const present=presentPlayers.length;
  const normalizedGroups=Array.from({length:teamCount},(_,i)=>(groups[i]??[]).filter(id=>presentPlayers.some(p=>p.id===id)));
  const assigned=new Set(normalizedGroups.flat());
  const unassigned=presentPlayers.filter(p=>!assigned.has(p.id));
  const setCount=(count:2|3)=>{setTeamCount(count);setGroups(g=>count===3?[g[0]??[],g[1]??[],g[2]??[]]:[g[0]??[],[...(g[1]??[]),...(g[2]??[])]]);};
  const movePlayer=(id:string,target:number)=>setGroups(g=>{const next=Array.from({length:teamCount},(_,i)=>(g[i]??[]).filter(x=>x!==id));if(target>=0)next[target].push(id);return next;});
  const autoSplit=()=>{const next=Array.from({length:teamCount},()=>[] as string[]);presentPlayers.forEach((p,i)=>next[i%teamCount].push(p.id));setGroups(next);};
  const playerName=(id:string)=>team.players.find(p=>p.id===id)?.name??'Spieler';
  return <div className="modal-wrap"><div className="modal wide"><div className="modal-head"><div><span>{training?'TRAINING BEARBEITEN':'NEUES TRAINING'}</span><h2>{fmt(date)}</h2></div><button className="icon-btn" onClick={close}><X/></button></div><div className="attendance-summary"><div><strong>{present}</strong><span>von {active.length} anwesend</span></div><div className="big-rate">{active.length?Math.round(present/active.length*100):0} %</div></div><div className="hint">Alle Spieler sind zunächst anwesend. Ändere nur die Abwesenden.</div><div className="status-list">{active.map(p=><div className="status-row" key={p.id}><strong>{p.name}</strong><div>{([['present','Anwesend'],['excused','Entschuldigt'],['unexcused','Unentsch.'],['injured','Verletzt']] as [Status,string][]).map(([v,l])=><button key={v} className={`${v} ${attendance[p.id]===v?'selected':''}`} onClick={()=>setAttendance(a=>({...a,[p.id]:v}))}>{l}</button>)}</div></div>)}</div>
  <div className="team-builder-toggle"><button className="secondary" type="button" onClick={()=>setShowTeams(v=>!v)}><Users size={18}/> {showTeams?'Teamaufteilung schließen':'Teams zusammenstellen'}</button></div>
  {showTeams&&<div className="team-builder"><div className="team-builder-head"><strong>Teams für Spielformen</strong><div><button type="button" className={teamCount===2?'primary':'secondary'} onClick={()=>setCount(2)}>2 Teams</button><button type="button" className={teamCount===3?'primary':'secondary'} onClick={()=>setCount(3)}>3 Teams</button><button type="button" className="secondary" onClick={autoSplit}>Automatisch verteilen</button></div></div><p className="team-builder-help">Tippe bei einem Spieler auf das gewünschte Team. Es werden nur anwesende Spieler angezeigt.</p>
    {unassigned.length>0&&<div className="unassigned"><strong>Noch nicht zugeteilt ({unassigned.length})</strong><div className="player-chips">{unassigned.map(p=><div className="player-chip" key={p.id}><span>{p.name}</span>{normalizedGroups.map((_,i)=><button type="button" key={i} onClick={()=>movePlayer(p.id,i)}>T{i+1}</button>)}</div>)}</div></div>}
    <div className={`team-columns teams-${teamCount}`}>{normalizedGroups.map((ids,i)=><div className="team-column" key={i}><h4>Team {i+1} <span>{ids.length}</span></h4>{ids.length?ids.map(id=><div className="team-player" key={id}><span>{playerName(id)}</span><div>{normalizedGroups.map((_,j)=>j!==i&&<button type="button" key={j} onClick={()=>movePlayer(id,j)}>T{j+1}</button>)}<button type="button" onClick={()=>movePlayer(id,-1)}>×</button></div></div>):<small>Noch keine Spieler</small>}</div>)}</div>
  </div>}
  <label className="note-label">Notiz (optional)<textarea maxLength={160} value={note} onChange={e=>setNote(e.target.value)} placeholder="z. B. Schwerpunkt, Besonderheiten …"/></label><div className="modal-actions">{remove&&<button className="delete" onClick={remove}><Trash2/> Löschen</button>}<span/><button className="secondary" onClick={close}>Abbrechen</button><button className="primary" onClick={()=>save({id:training?.id??uid(),date,note,attendance,teamGroups:normalizedGroups})}>Training speichern</button></div></div></div>
}
...
function backupAll(data:Data){const payload=JSON.stringify({app:'Trainingsbeteiligung',version:1,exportedAt:new Date().toISOString(),data},null,2);const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([payload],{type:'application/json'}));a.download=`Trainingsbeteiligung-Backup-${iso(new Date())}.json`;a.click();URL.revokeObjectURL(a.href)}
function restoreAll(file:File,setData:React.Dispatch<React.SetStateAction<Data>>,notify:(s:string)=>void){const reader=new FileReader();reader.onload=()=>{try{const parsed=JSON.parse(String(reader.result));const next=parsed?.data??parsed;if(!next||!Array.isArray(next.teams)||typeof next.selectedTeamId!=='string')throw new Error('invalid');if(!confirm(`Backup mit ${next.teams.length} Mannschaft(en) wiederherstellen? Die aktuellen App-Daten werden ersetzt.`))return;localStorage.setItem('trainingsbeteiligung-v1',JSON.stringify(next));setData(next);notify('Backup erfolgreich wiederhergestellt')}catch{alert('Diese Datei ist kein gültiges Trainingsbeteiligung-Backup.')}};reader.readAsText(file)}
function SettingsModal({team,data,close,updateTeam,setData,notify}:{team:Team;data:Data;close:()=>void;updateTeam:(f:(t:Team)=>Team)=>void;setData:React.Dispatch<React.SetStateAction<Data>>;notify:(s:string)=>void}){const[name,setName]=useState(team.name),[season,setSeason]=useState(team.season),[start,setStart]=useState(team.start),[end,setEnd]=useState(team.end),[newTeam,setNewTeam]=useState('');return <div className="modal-wrap"><div className="modal small"><div className="modal-head"><h2>Einstellungen</h2><button className="icon-btn" onClick={close}><X/></button></div><div className="form"><label>Mannschaftsname<input value={name} onChange={e=>setName(e.target.value)}/></label><label>Saisonbezeichnung<input value={season} onChange={e=>setSeason(e.target.value)}/></label><div className="form-row"><label>Saisonstart<input type="date" value={start} onChange={e=>setStart(e.target.value)}/></label><label>Saisonende<input type="date" value={end} onChange={e=>setEnd(e.target.value)}/></label></div><hr/><div><strong>Datensicherung</strong><p style={{margin:'6px 0 12px',opacity:.75}}>Sichert alle Mannschaften, Spieler, Trainings und Teamaufteilungen in einer Datei.</p><div className="inline-add"><button type="button" className="secondary" onClick={()=>{backupAll(data);notify('Komplettes Backup erstellt')}}><Download size={17}/> Backup speichern</button><label className="secondary" style={{cursor:'pointer',display:'inline-flex',alignItems:'center',gap:8}}><Upload size={17}/> Backup laden<input type="file" accept="application/json,.json" style={{display:'none'}} onChange={e=>{const f=e.target.files?.[0];if(f)restoreAll(f,setData,notify);e.currentTarget.value=''}}/></label></div></div><hr/><label>Neue Mannschaft<div className="inline-add"><input value={newTeam} onChange={e=>setNewTeam(e.target.value)} placeholder="z. B. U15"/><button className="secondary" disabled={!newTeam.trim()} onClick={()=>{const t=seedTeam(newTeam.trim(),data.teams.length*3);setData(d=>({...d,teams:[...d.teams,{...t,players:[],trainings:[]}],selectedTeamId:t.id}));close();notify('Mannschaft angelegt')}}>Anlegen</button></div></label></div><div className="modal-actions"><span/><button className="secondary" onClick={close}>Abbrechen</button><button className="primary" onClick={()=>{updateTeam(t=>({...t,name,season,start,end}));close();notify('Einstellungen gespeichert')}}>Speichern</button></div></div></div>}
