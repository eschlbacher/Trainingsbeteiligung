    ...
    <div className="app-shell">
      ...
      <main>
        ...
        <div className="content">
          ...
          {view==='players' && <PlayersView team={team} onAdd={()=>setModal('player')} updateTeam={updateTeam} notify={notify} onPerformance={p=>setPerformancePlayer(p)} onEdit={p=>{setEditingPlayer(p);setModal('player')}}/>}
      ...
      {modal==='teamBuilder' && teamBuilderTraining && <TeamBuilder team={team} training={teamBuilderTraining} close={()=>setModal('training')} save={tr=>{setTeamBuilderTraining(tr);updateTeam(t=>({...t,trainings:t.trainings.some(x=>x.id===tr.id)?t.trainings.map(x=>x.id===tr.id?tr:x):[...t.trainings,tr]}));setEditingTraining(tr);setTrainingDate(tr.date);setModal('training');notify('Teamaufteilung gespeichert');}}/>}
      {modal==='player' && <PlayerModal team={team} player={editingPlayer} close={()=>{setEditingPlayer(null);setModal(null)}} save={p=>{updateTeam(t=>({...t,players:editingPlayer?t.players.map(x=>x.id===p.id?p:x):[...t.players,p]}));setEditingPlayer(null);setModal(null);notify(editingPlayer?'Spieler geändert':'Spieler hinzugefügt');}}/>}
...
function PlayersView({team,onAdd,updateTeam,notify,onPerformance,onEdit}:{team:Team;onAdd:()=>void;updateTeam:(f:(t:Team)=>Team)=>void;notify:(s:string)=>void;onPerformance:(p:Player)=>void;onEdit:(p:Player)=>void}) {
...
function PlayerModal({team,player,close,save}:{team:Team;player?:Player|null;close:()=>void;save:(p:Player)=>void}){const[name,setName]=useState(player?.name??''),[from,setFrom]=useState(player?.activeFrom??team.start),[to,setTo]=useState(player?.activeTo??team.end);return <div className="modal-wrap"><div className="modal small"><div className="modal-head"><h2>{player?'Spieler bearbeiten':'Spieler hinzufügen'}</h2><button className="icon-btn" onClick={close}><X/></button></div><div className="form"><label>Name<input autoFocus value={name} onChange={e=>setName(e.target.value)} placeholder="Vor- und Nachname"/></label><div className="form-row"><label>Aktiv von<input type="date" value={from} onChange={e=>setFrom(e.target.value)}/></label><label>Aktiv bis<input type="date" value={to} onChange={e=>setTo(e.target.value)}/></label></div></div><div className="modal-actions"><span/><button className="secondary" onClick={close}>Abbrechen</button><button className="primary" disabled={!name.trim()} onClick={()=>save({...(player??{id:uid()}),name:name.trim(),activeFrom:from,activeTo:to})}>{player?'Änderungen speichern':'Hinzufügen'}</button></div></div></div>}
